  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="card" style={{
        ...
        <div className="textarea" style={{ display: 'grid', gridTemplateColumns: 'auto auto auto auto', fontFamily: '"Poppins"', fontWeight: 200 }}><TextField multiline
          rows={2} id="outlined-basic" label="Description(optional)" variant="outlined" style={{ width: '350px' }} /></div>
        ...
        <div className="inputrating" style={{ display: 'grid', gridTemplateColumns: '67% auto' }}>
          ...
          <Slider onChange={handleChangeRating}
            ...
            max={5}
            style={{ color: '#364cff' }}
          /><span className="rating" style={{ gridColumnStart: 4, gridColumnEnd: 5, textAlign: 'center', background: 'white', color: 'black', border: '0.1px solid rgba(0, 0, 0, 0.65)', boxShadow: '2px 2px 0px #000000', borderRadius: '12px', padding: '5px 20px 5px 20px', fontSize: '12px', fontFamily: '"Poppins"', fontWeight: 600 }}>{rating}</span>
        </div>
        <div className="inputrating" style={{ display: 'grid', gridTemplateColumns: '67% auto' }}>
...
const AddView = (props) => {
  ...
  const handleAddView = (newrating, currentParts, currentProp) => {
    axios
      .put(`http://localhost:3001/properties/${id}`, {
        ...currentProp,
        stars: newrating,
        parts: currentParts,
      })
  ...
  const handleSubmit = (event) => {
    ...
    let newrating;
    let newWeight = parseFloat(parseFloat(event.target[7].value).toFixed(1));
    ...
    currentParts = [
      ...currentProp.parts,
      {
        ...
        rating: parseFloat(parseFloat(event.target[6].value).toFixed(1))
      },
    ];

    numerator += parseFloat(parseFloat(event.target[6].value).toFixed(1)) * newWeight;
    denominator += newWeight;

    console.log(numerator, denominator);
    newrating = (numerator * 1.0 / denominator).toFixed(1);
    let finder = props.views.find(el => (el.name).toLowerCase() === (event.target[0].value).toLowerCase())
  ...
  if (props.show === false) {
    return (
      ...
      <svg onClick={() => props.setShow(!props.show)} xmlns="http://www.w3.org/2000/svg" width={70} height={70} style={{
        position: "fixed",
        bottom: "0",
        right: "0",
        marginRight: "50px",
