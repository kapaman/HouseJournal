...
const HouseCard2 = (props) => {
  ...
  return (
    ...
    <div class="higherHouseCard"
      style={{ display: 'flex',width:'min-content',justifySelf: 'center', justifyContent: 'center', textAlign: 'center', fontSize: '30px', margin: '20px 0', borderRadius: '10px', backgroundColor: 'white', padding: '5px' }}>
      <div className="view"
...
const Main = (props) => {
  ...
  const calculateRating = (views) => {
    ...
    for (let j = 0; j < newproperties.length; j++) {
      ...
      for (let i = 0; i < newproperties[j].parts.length; i++) {
        let finder = props.views.find(el => (el.name).toLowerCase() === (newproperties[j].parts[i].name).toLowerCase())
        if (finder != undefined) {
          // console.log(finder, newproperties[j].parts[i])
          numerator += parseFloat(finder.weight) * parseFloat(newproperties[j].parts[i].rating);
  ...
  return (
    <div className="flex-container"
      ...
      margin: '80px auto auto auto', gridTemplateColumns:'1fr 1fr 1fr', justifyContent: 'space-evenly',justifyItems:'center', backgroundColor: 'white', marginTop: '80px' }}>
      {newproperties.sort((a, b) => b.rating - a.rating).map(el => <HouseCard2 id={el._id} key={el._id} stars={el.rating} address={el.address} src={el.img}></HouseCard2>)}
...
const App = () => {
  ...
  useEffect(() => {
    axios
      .get('http://192.168.18.163:3001/properties')
      .then(response => {
  ...
  useEffect(() => {
    axios
      .get('http://192.168.18.163:3001/views')
      .then(response => {
        console.log(response.data);
      ...
      <div >
        <Switch >
          <Route path="/properties/:id">
            ...
            <Navi addNew="add View" show={showView} setShow={setShowView}></Navi>
          ...
          <Navi show={showProperty} setShow={setShowProperty} addNew="add Property"></Navi>
            {/* <NavBar show={showProperty} setShow={setShowProperty} addNew="add Property"></NavBar> */}
            <Main properties={properties.length > 0 ? properties : null} views={views.length > 0 ? views : null}></Main>
